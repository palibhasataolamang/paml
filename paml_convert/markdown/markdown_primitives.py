...
primitive_to_markdown_functions = (
    {}
)  # dictionary of identity : function for primitives markdown conversion
...
primitive_to_markdown_functions[
    LIQUID_HANDLING_PREFIX + "Provision"
] = liquid_handling_provision_to_markdown


def liquid_handling_dispense_to_markdown(executable, mdc: MarkdownConverter):
    volume = executable.input_pin("amount").to_markdown(mdc)
    source = mdc.protocol_typing.flow_values[
        executable.input_pin("source").input_flows().pop()
    ]
    resource = mdc.document.find(source.specification).to_markdown(
        mdc
    )  # Kludge due to document addition failures
    source_location = mdc.document.find(source.in_location[0]).to_markdown(
        mdc
    )  # TODO: generalize to support multi-locations
    destination = executable.input_pin("destination").to_markdown(mdc)
    return (
        "Pipette "
        + volume
...
primitive_to_markdown_functions[
    LIQUID_HANDLING_PREFIX + "Dispense"
] = liquid_handling_dispense_to_markdown
...
primitive_to_markdown_functions[
    LIQUID_HANDLING_PREFIX + "Transfer"
] = liquid_handling_transfer_to_markdown
...
def liquid_handling_transferinto_to_markdown(executable, mdc: MarkdownConverter):
    ...
    source = executable.input_pin("source").to_markdown(mdc)
    destination = executable.input_pin("destination").to_markdown(mdc)
    mix_cycles = executable.input_pin("mixCycles").to_markdown(
        mdc
    )  # TODO: this should be optional, not required
    return (
        "Pipette "
        + volume
        + " from "
        ...
        + mix_cycles
        + " times at destination\n"
    )


primitive_to_markdown_functions[
    LIQUID_HANDLING_PREFIX + "TransferInto"
] = liquid_handling_transferinto_to_markdown
...
primitive_to_markdown_functions[
    LIQUID_HANDLING_PREFIX + "PipetteMix"
] = liquid_handling_pipettemix_to_markdown
...
primitive_to_markdown_functions[
    PLATE_HANDLING_PREFIX + "Cover"
] = plate_handling_cover_to_markdown
...
primitive_to_markdown_functions[
    PLATE_HANDLING_PREFIX + "Seal"
] = plate_handling_seal_to_markdown
...
primitive_to_markdown_functions[
    PLATE_HANDLING_PREFIX + "Uncover"
] = plate_handling_uncover_to_markdown
...
primitive_to_markdown_functions[
    PLATE_HANDLING_PREFIX + "Unseal"
] = plate_handling_unseal_to_markdown
...
def plate_handling_incubate_to_markdown(executable, mdc: MarkdownConverter):
    ...
    shakingFrequency = executable.input_pin("shakingFrequency").to_markdown(
        mdc
...
primitive_to_markdown_functions[
    PLATE_HANDLING_PREFIX + "Incubate"
] = plate_handling_incubate_to_markdown
...
primitive_to_markdown_functions[
    SPECTROPHOTOMETRY + "MeasureAbsorbance"
] = spectrophotometry_absorbance_to_markdown
...
primitive_to_markdown_functions[
    SPECTROPHOTOMETRY + "MeasureFluorescence"
] = spectrophotometry_fluorescence_to_markdown
