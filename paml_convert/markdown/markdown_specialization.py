...
class MarkdownSpecialization(BehaviorSpecialization):
    def __init__(self, out_path) -> None:
        ...
        self.sequence = []
        self.sequenceDict = {}
        self.type = "MarkdownSpecialization"
        self.header = ""
    ...
    def on_begin(self):
        if self.execution:
            ...
            self.header += self.markdown_converter.markdown_header(protocol)
            self.header += self._materials_markdown(protocol)
            self.header += self._inputs_markdown(self.execution.parameter_values)
    ...
    def _materials_markdown(self, protocol):
        ...
        components = [
            x
            for x in protocol.document.objects
            if isinstance(x, sbol3.component.Component)
        ]
    ...
    def on_end(self):
        self.header += self._outputs_markdown(self.execution.parameter_values)
        ...
        for i in self.execution.parameter_values:
            ...
            if parameter.property_value.direction == uml.PARAMETER_OUT:
                output_parameters.append(
                    f"`{parameter.property_value.name}` from `{value}`"
                )
    ...
    def define_container(self, record: paml.ActivityNodeExecution):
        ...
        try:
            possible_container_types = (
                possible_container_types
            ) = self.resolve_container_spec(spec)
            containers_str = ",".join(
                [f"\n\t[{c.split('#')[1]}]({c})" for c in possible_container_types]
            )
            self.markdown_steps += [
                f"Provision a container named `{samples_var.name}` such as: {containers_str}."
            ]
            self.sequenceDict[
                call.identity
            ] = f"Provision a container named `{samples_var.name}` meeting specification: {spec.queryString}."
    ...
    def measure_absorbance(self, record: paml.ActivityNodeExecution):
        ...
        self.markdown_steps += [
            f"Make absorbance measurements (named `{measurements}`) of {samples_str} at {wl.value} {wl_units}."
        ]
        self.sequenceDict[
            call.identity
        ] = f"Make absorbance measurements (named `{measurements}`) of {samples_str} at {wl.value} {wl_units}."
