...
class OT2Specialization(BehaviorSpecialization):
    ...
    def on_begin(self):
        ...
        self.header += self._tipracks(
            json.loads(self.leftTipJson), json.loads(self.rightTipJson)
        )
        self.header += self._materials(protocol)

    def on_end(self):
        self.header += self.containerDependencies
    ...
    def provision_container(self, record: paml.ActivityNodeExecution):
        ...
        try:
            if velocityUnit == "Milliliter per Second":
                ...
                self.all_steps += [
                    f"#  Unsupported Unit {velocityUnit} for attribute dispenseVelocity"
                ]
                raise AttributeError("Unsupported Unit for attribute dispenseVelocity")
        except:
            velocity = 150
        destPlate = self.var_to_entity[
            destination.source.lookup().value.lookup().get_parent().display_id
        ]
    ...
    def transfer_into(self, record: paml.ActivityNodeExecution):
        ...
        try:
            if mixCyclesUnit == "Count":
                ...
                self.all_steps += [
                    f"#  Unsupported Unit {mixCyclesUnit} for attribute mixCycles"
                ]
        ...
        # units = tyto.OM.get_term_by_uri(units)
        sourcePlate = self.var_to_entity[
            source.source.lookup().value.lookup().get_parent().display_id
        ]
        destPlate = self.var_to_entity[
            destination.source.lookup().value.lookup().get_parent().display_id
        ]
